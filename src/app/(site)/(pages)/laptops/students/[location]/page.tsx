...
import {
  ...
  getStudentLaptopRegionalSeoContent,
...
export async function generateMetadata({
  ...
  const content = getStudentLaptopRegionalSeoContent(locale, locationData.slug);

  return buildSeoMetadata({
    title: content.metaTitle,
    description: content.metaDescription,
    path: buildLaptopStudentsLocationPath(locationData.slug),
    keywords: content.keywords,
  });
}

export default async function LaptopStudentsLocationPage({
  params,
}: PageParams) {
  const { location } = await params;
