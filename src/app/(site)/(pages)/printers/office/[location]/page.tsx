...
import {
  getOfficePrinterRegionalSeoContent,
...
export async function generateMetadata({
  ...
  const content = getOfficePrinterRegionalSeoContent(locale, locationData.slug);

  return buildSeoMetadata({
    title: content.metaTitle,
    description: content.metaDescription,
    path: buildOfficePrinterLocationPath(locationData.slug),
    keywords: content.keywords,
  });
}

export default async function OfficePrinterLocationPage({
  params,
}: PageParams) {
  const { location } = await params;
